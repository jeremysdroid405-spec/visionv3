...
- ✅ **Auto-Optimizer DONE (2026-05-21)** — large-scale threshold grid search now self-service end-to-end.
  ...
  - Metrics per combo: n_bets, wins/losses/pushes, hit_rate, ROI (American-odds → unit P&L), calibration_delta, avg TP/CV/edge, daily_consistency, max_drawdown_units, profit_units, n_days. Composite balanced score penalizes low samples + high drawdown + poor consistency.
  - HARD-FAILS upfront if replay cache empty in window (409 with explicit "Run SSOT pipeline first" message).
  - Verified end-to-end via 600-row synthetic seed: 54 cells × 40 combos in <2s, top result identified correctly, 5 candidates persisted, seed cleaned up.
  - **Frontend** Optimizer tab (`/admin/testing` tab 2): full config form (scope + tiers/sides + comma-separated grid values per axis + required filters) → progress card (bar, tested/total, cells done, skipped low-sample, failures, best-so-far inline) → results panel (Best-by-Tier cards, Top-25 ranked table with overfit-flag highlighting, Best-by-Family/Bucket lists, Worst configs, action buttons: Save top-K → candidates / Set as Testing Default / Export JSON).
  - **Sweep tab integration**: on mount, fetches `/optimizer/_meta/testing_default` and (if present and no existing draft is active) preloads the best optimizer thresholds into every stat_family's threshold row. User can still toggle families on/off and tweak individual axes.
  - Lint clean (frontend), `yarn build` clean (237.85 kB main bundle), locked-state render verified.
...
- ✅ **SSOT Audit UI DONE (2026-05-21)** — `/admin/testing` now exposes the SSOT diff harness end-to-end.
  ...
  - **Results tab**: new `DiffSummarySection` rendered above the main dashboard. Selector lists last 30 diff runs (sorted desc by `diff_emitted_at`). Headline stat cards: total sampled, tier-decision changes, SH/FL/WZ flips (with +promoted / -demoted breakdown), missing-in-SSOT count. Plain-English verdict ("✓ parity confirmed" vs "⚠ N changed"). Reason-code Venn: only-in-legacy, only-in-SSOT, shared with Δ. Up to 20 examples of changed decisions with side-by-side legacy → SSOT tier and failed-reasons. All driven by `/collections/.../aggregate` and `/find`.
- ✅ **P0 — Replay SSOT Refactor DONE (2026-05-21)** — `scripts/sgo/historical_full_pipeline_replay.py` completely rewritten (727 → 566 lines). **Zero inlined gate logic remains.**
  - All `_SH_SPEC`/`_FL_SPEC`/`_WZ_SPEC` constants and `eval_safe_haven`/`eval_front_lines`/`eval_war_zone` functions deleted. `_hit_rate_panels` / `_extract_stat_vals_from_game_logs` deleted. `_FAMILY_TO_HF` mapping deleted. `MLBHighFrictionModel.predict()` direct orchestration deleted.
  - Now delegates entirely to `services.replay.production_replay_runner.run_production_replay(gate_path="universal", output_namespace="propvision_full_pipeline")` — the SAME path that grades live PrizePicks props. One runner call per (date × tier).
  - **HARD-FAIL** by default on any runner error or missing SSOT input (`sgo_replay_alt_odds_raw` empty, `sgo_pp_research_outcomes` empty). `--continue-on-error` override required to skip bad cells.
  - **NO silent fallback** to inlined gates — preflight raises `RuntimeError` if reshape hasn't run first, with explicit message naming the fix-job.
  - Post-run mirror to legacy `sgo_propvision_full_pipeline_replay` (preserves UI Results panel + grid_sweep compatibility) collapses all 3 tier evaluations per prop and attaches outcome from `sgo_pp_research_outcomes`. New audit fields: `ssot_source="production_replay_runner"`, `runner_serials=[…]`, per-tier `*_gate_cfg_version` from the production threshold SHA.
  - `--sample-diff N` mode snapshots N random pre-existing legacy rows BEFORE the run, then emits per-row diff (legacy decision vs SSOT decision, including tier deltas) to `sgo_propvision_full_pipeline_replay_diff`. Legacy gate code is NEVER re-executed.
  - Legacy CLI flags (`--exclude-stat-family`, `--limit`, `--force`) retained as documented no-ops so the existing UI Workflow tab + Admin API allowlist continue to function without changes.
  - Diagnostics `preflight.py` updated to track `sgo_replay_alt_odds_raw` and warn with `no_reshape_odds` (high severity) when empty — fix-job: `scripts.sgo.reshape_sgo_to_replay_odds`. Surfaces in the UI Diagnostics tab with one-click "Run Fix".
  - **Verified**: parse OK, import OK, preflight HARD-FAIL with exit=1 when SSOT inputs missing. End-to-end run on real data NOT yet executed (requires production data which the preview pod does not host).
- ✅ **Internal Quant Terminal v3 DONE (2026-05-21)** — full self-service research platform at `/admin/testing`. Now tabbed (Workflow / Sweep / Results / Candidates / Models / Diagnostics).
  - **New backend routers**: `routes/emergent_admin/preflight.py` (`GET /preflight/` — dependency / collection / model-artifact health snapshot with classified warnings + fix-job mapping) and `routes/emergent_admin/models.py` (universal model registry CRUD: register / clone / validate / activate / archive). Backed by writable `emergent_model_registry` collection (added to `policy.WRITABLE_COLLECTIONS`).
  - **Frontend additions**: Lock Page button + PRIVATE INTERNAL TESTING TOOL warning banner. Sport selector wired into `SPORT_ADAPTERS` map (MLB live, NBA/NFL frontend-ready with adapter coverage indicators showing missing backend jobs). Per-Stat Sweep Builder restored with all 12 axes per family (hr_l20/l10/l5, cv, edge, tp, projection_margin, vision_score, sharp_book_count, devig_book_count, market_width, consensus_disagreement). Workflow steps now show runtime + plain-English "next recommended action" hints. Diagnostics tab surfaces classified warnings with one-click "Run Fix" → posts the exact remediation job to `/jobs/run`. Models tab full CRUD UI.
  - Backend smoke-tested via curl: preflight returns 7 warnings + 15 tracked collections + MLB/NBA/NFL artifact dirs; model register/list endpoints verified.
  - File: `/app/frontend/src/pages/AdminTesting.jsx` (1,300+ lines, lint clean). Locked-state render verified.
- ⚠️ **OUTSTANDING — Replay SSOT Audit (P0)** — `scripts/sgo/historical_full_pipeline_replay.py` still **inlines** its own SH/FL/WZ gate evaluators (lines 245-363) and its own `_hit_rate_panels` — that violates the user directive "NO alternate historical-only logic". Needs a Phase-2 backend refactor to delete the inlined evaluators and delegate to `services/replay/production_replay_runner.run_production_replay` (already exists and is the live-pipeline SSOT). UI is ready to consume that; backend hasn't been re-wired yet.
- ✅ **Canonical-name backfill script DONE (2026-05-21)** — `scripts/research/backfill_stat_family_canonical.py` normalizes legacy `stat_family` values on `mlb_replay_feature_cache` and `mlb_replay_model_outputs` using `canonical_stats.canonical_family()`. Default `--dry-run`; `--commit` performs idempotent bulk writes in chunks; legacy value preserved in `stat_family_legacy` for audit. Registered as enabled in `policy.ALLOWED_JOBS["scripts.research.backfill_stat_family_canonical"]`. Verified end-to-end: synthetic seed → 4/6 rows correctly mapped (`strikeouts→batter_strikeouts`, `pitcher_walks→walks_allowed`, `hits+runs+rbis→hits_runs_rbis`, `Hits→hits`), 1/6 already canonical, 1/6 blank skipped; re-run shows 0 updates (idempotent); execution via Admin API confirmed.
- ✅ **Replay card metadata propagation DONE (2026-05-21)** — `event_id`, `commence_time`, `game_date` now stamped onto every card in `services/picks/card_builder.py::_build_card_record` + `ProductionReplayCard` schema. Tests 16/16 green.
- ✅ **Emergent Admin API DONE (2026-05-21)** — `/api/emergent-admin/*` mounted in `server.py`. Token-auth, RBAC, allowlisted job runner, scoped service restart, audit log. Reference: `/app/memory/EMERGENT_ADMIN_API.md`.
