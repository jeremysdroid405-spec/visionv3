...
- ✅ **Private Universal Historical Testing Command Center v2 DONE (2026-05-21)** — full self-service rebuild at `/admin/testing`. Token-gated, unlinked.
  - **Guided 7-step workflow** (Backfill stats → Build features → Score model → Full pipeline replay → Grid sweep → View results → Save candidate) with visual step cards, status pills, and live log tail.
  - **Auto job chaining** — single "Run Full Replay Pipeline" button drives the entire chain via `/jobs/run` + `/jobs/{id}` polling; pipeline state persisted to `localStorage[emergentAdminPipeline]` so refresh doesn't lose progress.
  - **Real Results Panel** — auto-loads on completion: queries `/collections/research_grid_runs/find` + `research_grid_results/find` + `sgo_propvision_full_pipeline_replay/aggregate` for ROI/HR/TP/CV/edge per tier · family · odds bucket · side. Computes max drawdown from daily ROI sequence. Reason-code histogram from gate-fail arrays.
  - **Plain-English summary** — Best SH / FL / WZ config, best/worst stat family, best odds bucket, "DO NOT USE" worst configs.
  - **Candidate Manager** — save winning rows with name + tag (review/ready/reject), compare up to 4 side-by-side, export JSON, "Mark Ready" → POST `/configs/draft` (persists to `emergent_candidate_configs`).
  - File: `/app/frontend/src/pages/AdminTesting.jsx`. Routed in `App.js`. Lint clean.
