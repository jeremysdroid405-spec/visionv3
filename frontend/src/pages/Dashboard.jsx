          ...
          {[...scores, ...scores].map((game, idx) => {
            ...
            return (
              <div key={`score-${idx}`} className={`flex items-center gap-2.5 px-4 py-1.5 border-r border-zinc-700/50 ${isLive ? 'bg-zinc-800/50' : ''}`}>
                ...
                <span className={`text-base font-black ${game.away_score > game.home_score ? 'text-yellow-500' : 'text-red-400'}`}>
                  {game.away_score}
                </span>
                ...
                {/* Home Team */}
                <span className={`text-base font-black ${game.home_score > game.away_score ? 'text-yellow-500' : 'text-red-400'}`}>
                  {game.home_score}
                </span>
                <span className="text-sm font-bold text-white">
                  {game.home_team}
