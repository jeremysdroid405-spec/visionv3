...
async def run_board_intelligence_enrichment(db: AsyncIOMotorDatabase) -> Dict[str, Any]:
    ...
    cached_board = COLL.handle(db, "board_cache", "nba")
...
async def _load_market_edge_data(
    ...
    try:
        ...
        event_ids = set()
        async for player in COLL.handle(db, "board_cache", "nba").find({}, {"props.event_id": 1}):
...
async def _find_prop_index(db: AsyncIOMotorDatabase, pick: Dict) -> int:
    ...
    player = await COLL.handle(db, "board_cache", "nba").find_one(
...
async def _enrich_pick_full(
    ...
    try:
        ...
        update_result = await COLL.handle(db, "board_cache", "nba").update_one(
            {"player_name": player_name},
            {"$set": update_data}
        )
