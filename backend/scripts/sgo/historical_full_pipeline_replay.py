...
async def _mirror_to_legacy(db, *, replay_serials: List[str],
    ...
    if not replay_serials:
        return (0, 0)
    # 2026-06-02 — Pre-filter to event_ids that exist in the outcomes
    # collection for the same window. RCA: `mlb_propvision_full_pipeline_outputs`
    # accumulated rows from TWO Layer-3 sources — the SGO replay path
    # (V1 20-char event_ids matching `sgo_pp_research_outcomes`) and
    # the live production replay path (V2 32-char MD5 hash event_ids
    # generated by `mlb_historical_alt_odds_raw`). The V2-hash rows
    # have NO matching outcome row and cannot be graded; mirroring
    # them inflates the replay collection ~2x and drops grading
    # coverage from 97% to 33%. Restrict the mirror aggregation to
    ...
    async for r in db[RUNNER_RUNS].find(
        ...
        if gd:
            game_dates.add(gd)
    valid_event_ids: set = set()
    if game_dates:
        async for o in db[SGO_OUTCOMES_COLL].find(
            {"league_id": league.upper(),
             "game_date": {"$in": sorted(game_dates)},
             "outcome_resolved": True},
    ...
    print(f"  [mirror] event_id allowlist: {len(valid_event_ids):,} "
          f"event_ids in {SGO_OUTCOMES_COLL} for "
          f"{len(game_dates)} game_dates")
