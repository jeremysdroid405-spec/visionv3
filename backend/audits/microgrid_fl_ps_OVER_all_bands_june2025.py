"""
Micro-grid sweep — FL `pitcher_strikeouts` OVER side, across ALL FL odds bands.

Window: 2025-06-01 → 2025-06-30 (June 2025 cross-regime replay)
Side: OVER
Bands (every FL-applicable):
    [-500,-300]  [-299,-200]  [-199,-150]  [-149,-110]
    [-109,+100]  [+101,+200]  [+201,+inf]

Filter grid (full cross — identical to BS template):
...
Locked thresholds NOT changed; this is a search for any qualified combo
in the June 2025 regime. NO production-gate modification.
"""
...
async def main():
    ...
    c.close()
    print("══════ FL pitcher_strikeouts OVER — June 2025 pool overview ══════")
    total_n=total_g=0
    for b in BAND_ORDER:
        n = len(by_band.get(b, []))
    ...
    out_csv = f"/app/backend/audits/microgrid_fl_ps_OVER_all_bands_june2025_{stamp}.csv"
