...
Side: OVER (UNDER has effectively zero coverage on hits OVER props)
...
async def main():
    ...
    c.close()
    print("══════ FL hits OVER — pool overview ══════")
    total_n=total_g=0
    for b in BAND_ORDER:
        n = len(by_band.get(b, []))
    ...
    out_csv = f"/app/backend/audits/microgrid_fl_hits_OVER_all_bands_{stamp}.csv"
