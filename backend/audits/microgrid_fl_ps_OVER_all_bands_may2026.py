...
Window: 2026-05-03 → 2026-05-15  (Phase-1 LOCKED May 2026 window)
Side: OVER
Bands:
    [-500,-300]  [-299,-200]  [-199,-150]  [-149,-110]
    [-109,+100]  [+101,+200]  [+201,+inf]

Filter grid (identical to June 2025 sister script for direct comparison):
...
async def main():
    ...
    serials = [f"GSS-MLB-202605{d:02d}-FRON-POOL" for d in range(3,16)]
    ...
    c.close()
    print("══════ FL pitcher_strikeouts OVER — May 2026 (2026-05-03→05-15) pool overview ══════")
    total_n=total_g=0
    for b in BAND_ORDER:
        n = len(by_band.get(b, []))
    ...
    out_csv = f"/app/backend/audits/microgrid_fl_ps_OVER_all_bands_may2026_{stamp}.csv"
